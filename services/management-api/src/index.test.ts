...
beforeEach(() => {
  ...
  process.env.DESTINATIONS_TABLE_NAME = 'test-destinations-table';
  process.env.SEGMENTS_TABLE_NAME = 'test-segments-table';
  process.env.SEGMENT_MEMBERS_TABLE_NAME = 'test-segment-members-table';
...
describe('management-api', () => {
  it('lists sources', async () => {
    dynamoMock.on(ScanCommand).resolves({ Items: [{ id: 's1', name: 'Web' }] });
  ...
  it('gets a profile with events', async () => {
    dynamoMock.on(QueryCommand).resolves({
      Items: [
        { userId: 'user_1', sortKey: 'META', updatedAt: '2024-01-01T00:00:00Z' },
        { userId: 'user_1', sortKey: 'EVENT#2024-01-01T00:00:00Z#msg_1', type: 'track', event: 'Click', timestamp: '2024-01-01T00:00:00Z' },
      ],
    });
    const { handler } = await import('./index');
